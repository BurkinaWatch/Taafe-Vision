...
const buttonVariants = cva(
  ...
  " transition-[transform,box-shadow,background-color,border-color] duration-200 hover:-translate-y-0.5 hover:shadow-lg active:translate-y-0 active:shadow-sm",
  {
    variants: {
      variant: {
        default:
          "bg-primary text-primary-foreground border border-primary/20 shadow-[0_14px_30px_-18px_hsl(var(--primary))]",
        destructive:
          "bg-destructive text-destructive-foreground border border-destructive/20",
        outline:
          // Shows the background color of whatever card / sidebar / accent background it is inside of.
          // Inherits the current text color.
          "border border-foreground/20 bg-card/60 shadow-sm hover:border-primary/50 hover:bg-primary/10 active:shadow-none",
        secondary: "border border-secondary/70 bg-secondary text-secondary-foreground shadow-sm",
        // Add a transparent border so that when someone toggles a border on later, it doesn't shift layout/size.
        ghost: "border border-transparent hover:bg-primary/10 hover:text-foreground",
      },
