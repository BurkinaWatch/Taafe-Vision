...
**How to apply:** When changing dependencies, regenerate and validate `pnpm-lock.yaml` with pnpm 9.15.9 in non-interactive CI mode (`CI=1`); leave `package.json` without a `packageManager` field and let each environment select its supported pnpm runtime. A frozen install can pass interactively yet fail in CI when the lockfile's `overrides` metadata was generated by another pnpm major.